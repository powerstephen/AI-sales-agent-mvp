...
export default function LeadPage() {
  ...
  const lead = useMemo(() => {
    const fromUploaded = activeLeads.find((item) => item.id === params.id);
    if (fromUploaded) return fromUploaded;
    return getLeadById(params.id);
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600">
                Short, timing-aware messaging based on fit, signal and prior activity.
              </p>
            </div>
            ...
            <div className="flex gap-2">
              {["3 days", "5 days", "7 days"].map((delay) => (
                <button
                  key={delay}
                  onClick={() => setFollowUpTiming(delay)}
                  className={`rounded-full px-3 py-1 text-xs ${
                    followUpTiming === delay
                      ? "bg-black text-white"
          ...
          <div className="mt-6 space-y-6">
            ...
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5">
              <div className="mb-3 text-xs font-medium uppercase tracking-wide text-gray-500">
                Email 2 · Follow-up ({followUpTiming})
              </div>
