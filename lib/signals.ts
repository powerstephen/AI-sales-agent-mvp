...
export function getSignalDrivers(signal: Signal) {
  ...
  if (signal.signal.toLowerCase().includes("hiring")) {
    drivers.push("Hiring usually creates pressure to build or manage pipeline faster");
  }

  if (signal.signal.toLowerCase().includes("funding")) {
    drivers.push("Recent funding typically increases pressure to show commercial momentum");
  }

  if (signal.signal.toLowerCase().includes("expanding")) {
    drivers.push("Expansion often creates a need for better prioritisation and pipeline coverage");
  }

  if (signal.industry.toLowerCase().includes("saas")) {
    drivers.push("The company sits in a segment that already aligns with the strongest ICP");
  }
  ...
  if (
    ...
    drivers.push("The likely buyer persona is close to the current revenue-backed ICP");
  }
...
export function getSignalReasoning(signal: Signal) {
  ...
  const strength =
    score >= 80
      ...
      : "This is a weaker signal, but there may still be some outbound value.";
  ...
  const driver =
    drivers.length > 0
      ? `The likely commercial pressure is that ${drivers[0].toLowerCase()}.`
      : "";
...
export function getSignalRecommendedAction(signal: Signal) {
  ...
  if (score >= 70) {
    return {
      label: "Test with short sequence",
      reason: "Strong enough to work, but best handled with a tight two-step sequence.",
    };
  }

  return {
    label: "Monitor for stronger signal",
    reason: "Interesting account, but timing or fit is not yet strong enough for priority outreach.",
  };
}
